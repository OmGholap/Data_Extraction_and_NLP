...
for filename in os.listdir(folder_path):
    ...
    with open(path, 'r' ,encoding="utf-8") as file:
        ...
        def count_personal_pronouns(filename):
            # define regex pattern for personal pronouns
            pattern = r"\b(I|we|my|ours|us)\b"

            # find all matches of pattern in text_file
            ...
            # Replace with the filename of your text file
        filename = path
        ...
        def save_2_to_excel(func_list,x):
            for index, value in enumerate(func_list):
                worksheet.cell( row = index +2 , column = x, value = value)
            workbook.save('Output Data Structure.xlsx')

        save_2_to_excel(Personal_Pronouns,14)
        save_2_to_excel(Average_Word_Length,15)
